...
export const getHospitalsByCity = async (city) => {
  const res = await axiosInstance.get(`/hospitals/city/${encodeURIComponent(city)}`);
  return res.data;
};

export const getHospitalsBySpeciality = async (speciality) => {
  const res = await axiosInstance.get(`/hospitals/speciality/${encodeURIComponent(speciality)}`);
  return res.data;
};
