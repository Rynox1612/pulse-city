...
const getProfile = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id)
    .select("-password")
    .populate("savedHospitals", "name city contactNumber emergencyAvailable");
  if (!user)
    return res
      .status(404)
...
const getMedicalInfo = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id).select(
    "bloodGroup allergies chronicDiseases emergencyContact",
  );
  if (!user)
    return res
      .status(404)
  ...
  res
    .status(200)
    .json({ success: true, message: "Medical info fetched.", data: user });
});
...
const updateMedicalInfo = asyncHandler(async (req, res) => {
  ...
  ).select("bloodGroup allergies chronicDiseases emergencyContact");
...
const getSavedHospitals = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id).populate(
    "savedHospitals",
    "name city address contactNumber emergencyAvailable icuBeds erBeds",
  );
  if (!user)
    return res
      .status(404)
  ...
  res.status(200).json({
    success: true,
    message: "Saved hospitals fetched.",
...
const saveHospital = asyncHandler(async (req, res) => {
  ...
  if (!hospital)
    return res
      .status(404)
      .json({ success: false, message: "Hospital not found.", data: {} });

  const user = await User.findById(req.user.id);
  if (user.savedHospitals.includes(hospitalId)) {
    return res
      .status(409)
      .json({ success: false, message: "Hospital already saved.", data: {} });
  }
  ...
  res
    .status(200)
    .json({ success: true, message: "Hospital saved successfully.", data: {} });
});
